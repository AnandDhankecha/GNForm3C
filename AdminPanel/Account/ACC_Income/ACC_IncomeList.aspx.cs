...
public partial class AdminPanel_ACC_Income_ACC_IncomeList : System.Web.UI.Page
{
    ...
    protected void btnDeleteSelected_Click(object sender, EventArgs e)
    {
        ...
        // Loop through the Repeater items to get selected records
        foreach (RepeaterItem item in rpData.Items)
        {
            CheckBox chkSelect = (CheckBox)item.FindControl("chkSelect");
            if (chkSelect != null && chkSelect.Checked)
            {
                // Extract the IncomeID from the command argument
                HiddenField hfIncomeId = (HiddenField)item.FindControl("hfIncomeId");
                if (hfIncomeId != null)
                {
                    int incomeId;
                    if (int.TryParse(hfIncomeId.Value, out incomeId))
                    {
        ...
        {
            // Call your method to delete records based on the selected IDs
            DeleteRecords(selectedIds);
        }
    }

    private void DeleteRecords(List<int> ids)
    {
        ACC_IncomeDAL aCC_IncomeDAL = new ACC_IncomeDAL();
        // Implement your logic to delete records from the database
        foreach (int id in ids)
        {
            aCC_IncomeDAL.Delete(id);
        }
    ...
    {
        // Your logic to bind the repeater with data
    }
